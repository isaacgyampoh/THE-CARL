...
public class AuthenticationTests
{
    ...
    public async Task SameEmailInTwoOrganizations_ResolvesToTheCorrectUser()
    {
        ...
        await auth.RegisterUserAsync(new RegisterUserRequest(
            orgA.Id, branchA.Id, "Ama Mensah", "shared@carl.test", "Alpha-Passphrase-1", null, [CarlRoles.Agent]));
        await auth.RegisterUserAsync(new RegisterUserRequest(
            orgB.Id, branchB.Id, "Kofi Boateng", "shared@carl.test", "Beta-Passphrase-2", null, [CarlRoles.Agent]));
    ...
    public async Task RefreshTokensAreNotStoredInClearText()
    {
        ...
        await auth.RegisterUserAsync(new RegisterUserRequest(
            org.Id, branch.Id, "Yaa Asantewaa", "yaa@carl.test", "Str0ng-Passphrase!", null, [CarlRoles.Agent]));
    ...
    public async Task RotatedRefreshTokenCannotBeUsedTwice()
    {
        ...
        await auth.RegisterUserAsync(new RegisterUserRequest(
            org.Id, branch.Id, "Kwame Nkrumah", "kwame@carl.test", "Str0ng-Passphrase!", null, [CarlRoles.Agent]));
    ...
    public async Task ReusingARotatedTokenRevokesTheWholeFamily()
    {
        ...
        await auth.RegisterUserAsync(new RegisterUserRequest(
            org.Id, branch.Id, "Efua Sutherland", "efua@carl.test", "Str0ng-Passphrase!", null, [CarlRoles.Agent]));
    ...
    public async Task RefreshTokenCannotBeRedeemedAgainstAnotherOrganization()
    {
        ...
        await auth.RegisterUserAsync(new RegisterUserRequest(
            org.Id, branch.Id, "Nii Lamptey", "nii@carl.test", "Str0ng-Passphrase!", null, [CarlRoles.Agent]));
    ...
    public async Task RepeatedFailedLoginsLockTheAccountAndRaiseAnAlert()
    {
        ...
        await auth.RegisterUserAsync(new RegisterUserRequest(
            org.Id, branch.Id, "Abena Osei", "abena@carl.test", "Str0ng-Passphrase!", null, [CarlRoles.Agent]));
    ...
    public async Task WeakPasswordsAreRejected(string password)
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() => auth.RegisterUserAsync(new RegisterUserRequest(
            org.Id, branch.Id, "Weak Password", "weak@carl.test", password, null, [CarlRoles.Agent])));
    }
    ...
    public async Task LegacyRoleNamesMapOntoCanonicalRoles()
    {
        ...
        Assert.Contains(CarlRoles.Owner, user.Roles);
    }
    ...
    public async Task OneRoleCanBeSharedByManyUsersInTheSameOrganization()
    {
        ...
        var first = await auth.RegisterUserAsync(new RegisterUserRequest(
            org.Id, branch.Id, "First Agent", "first@carl.test", "Str0ng-Passphrase!", null, [CarlRoles.Agent]));
        var second = await auth.RegisterUserAsync(new RegisterUserRequest(
            org.Id, branch.Id, "Second Agent", "second@carl.test", "Str0ng-Passphrase!", null, [CarlRoles.Agent]));

        Assert.Contains(CarlRoles.Agent, first.Roles);
        Assert.Contains(CarlRoles.Agent, second.Roles);

        var reloaded = await auth.GetUsersAsync(org.Id);
        Assert.All(reloaded, u => Assert.Contains(CarlRoles.Agent, u.Roles));
        Assert.Equal(1, await db.Roles.CountAsync(r => r.OrganizationId == org.Id && r.Name == CarlRoles.Agent));
    }
    ...
    public async Task BranchScopedRoleRequiresABranch()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() => auth.RegisterUserAsync(new RegisterUserRequest(
            org.Id, null, "Floating Agent", "floating@carl.test", "Str0ng-Passphrase!", null, [CarlRoles.Agent])));
    }
    ...
    public async Task StaffCannotBeAttachedToAnotherOrganizationsBranch()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() => auth.RegisterUserAsync(new RegisterUserRequest(
            orgA.Id, foreignBranch.Id, "Cross Tenant", "cross@carl.test", "Str0ng-Passphrase!", null, [CarlRoles.Agent])));
    }
