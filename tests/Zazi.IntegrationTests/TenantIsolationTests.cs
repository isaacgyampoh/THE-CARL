...
/// </summary>
public class TenantIsolationTests : IClassFixture<ZaziApiFactory>
{
    private readonly ZaziApiFactory _factory;

    public TenantIsolationTests(ZaziApiFactory factory) => _factory = factory;
    ...
    public async Task TransactionList_ReturnsOnlyTheCallersOrganization()
    {
        ...
        var client = _factory.CreateClient().Authenticated(
            TestTokens.Create(alpha.OwnerUserId, alpha.OrganizationId, null, ZaziRoles.Owner));
    ...
    public async Task AgentCannotReachAnotherBranchWithinTheirOwnOrganization()
    {
        ...
        var client = _factory.CreateClient().Authenticated(
            TestTokens.Create(tenant.AgentUserId, tenant.OrganizationId, tenant.BranchAId, ZaziRoles.Agent));
    ...
    public async Task TokenSignedWithAnotherKey_IsRejected()
    {
        ...
        var client = _factory.CreateClient().Authenticated(
            TestTokens.CreateWithForeignKey(tenant.OwnerUserId, tenant.OrganizationId, null, ZaziRoles.Owner));
    ...
    public async Task SmsEvidenceIsRecordedAgainstTheCallersOrganizationOnly()
    {
        ...
        var client = _factory.CreateClient().Authenticated(
            TestTokens.Create(alpha.AgentUserId, alpha.OrganizationId, alpha.BranchAId, ZaziRoles.Agent));
    ...
    public async Task IdenticalSmsInTwoTenantsBothProduceEvidence()
    {
        ...
        var betaClient = _factory.CreateClient().Authenticated(
            TestTokens.Create(beta.AgentUserId, beta.OrganizationId, beta.BranchAId, ZaziRoles.Agent));
