...
/// </summary>
public class RoleAuthorizationTests : IClassFixture<ZaziApiFactory>
{
    private readonly ZaziApiFactory _factory;

    public RoleAuthorizationTests(ZaziApiFactory factory) => _factory = factory;
    ...
    public async Task OwnerCanReachTheOrganizationDashboard()
    {
        ...
        var client = _factory.CreateClient().Authenticated(
            TestTokens.Create(tenant.OwnerUserId, tenant.OrganizationId, null, ZaziRoles.Owner));
    ...
    public async Task AuditorIsReadOnly()
    {
        ...
        var client = _factory.CreateClient().Authenticated(
            TestTokens.Create(tenant.OwnerUserId, tenant.OrganizationId, null, ZaziRoles.Auditor));
    ...
    public async Task BranchManagerCannotEscalateByCreatingAnOwner()
    {
        ...
        var client = _factory.CreateClient().Authenticated(
            TestTokens.Create(tenant.AgentUserId, tenant.OrganizationId, tenant.BranchAId, ZaziRoles.BranchManager));
