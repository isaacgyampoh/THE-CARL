...
public class PostgresTenantIsolationTests
{
    ...
    public async Task TenantACannotCreateTransactionsForTenantB()
    {
        ...
        await using var db = _postgres.CreateContext();
        var currentUser = new StubCurrentUser(alpha.OrganizationId, alpha.UserId, alpha.BranchId, ZaziRoles.Owner);
    ...
    public async Task TenantACannotSynchroniseTenantBDevices()
    {
        ...
        var guard = new TenantGuard(
            db, new StubCurrentUser(alpha.OrganizationId, alpha.UserId, alpha.BranchId, ZaziRoles.Owner));
    ...
    private sealed class StubCurrentUser : ICurrentUserContext
    {
        ...
        public bool HasOrganizationWideScope => _roles.Any(ZaziRoles.IsOrganizationWide);
