...
/**
 * Capture must keep running while the app is locked, closed, or never opened.
 *
 * <p>The lock is a convenience: it stops a phone left on a counter from handing over the
 * business's records. If it ever came to gate capture as well, an agent's transactions would
 * stop being recorded whenever they put the phone down — which is most of the day — and
 * nothing on any screen would say so. The money would simply not be there at closing time.</p>
 *
...
class CaptureIsIndependentOfTheUiTest {
    ...
    @Test
    fun `nothing in the capture path knows about the lock or the activity`() {
        val offenders = captureSources.filter { file ->
            val source = file.readText()
            source.contains("AppLock") ||
                source.contains("MainActivity") ||
                source.contains("LockScreen")
        }
