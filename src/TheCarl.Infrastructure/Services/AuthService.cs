...
public class AuthService : IAuthService
{
    ...
    public async Task<UserDto> RegisterUserAsync(RegisterUserRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var requestedRoles = request.Roles.Length == 0 ? [CarlRoles.Agent] : request.Roles;
        var canonicalRoles = new List<string>();
        foreach (var supplied in requestedRoles)
        {
            var canonical = CarlRoles.Normalize(supplied)
        ...
        if (request.BranchId is null && canonicalRoles.Any(r => !CarlRoles.IsOrganizationWide(r)))
        {
    ...
    private static IReadOnlyList<string> ResolveCanonicalRoles(User user) =>
        user.Roles
            .Select(r => CarlRoles.Normalize(r.Name))
    ...
    /// Runtime authorization is driven by <see cref="CarlPolicies.RolesByPolicy"/>; this
    ...
    private string CreateAccessToken(User user, IReadOnlyList<string> canonicalRoles)
    {
        ...
        var claims = new List<Claim>
        {
            ...
            new(CarlClaimTypes.OrganizationId, user.OrganizationId.ToString()),
            new(CarlClaimTypes.SecurityStamp, user.SecurityStamp),
        ...
        {
            claims.Add(new Claim(CarlClaimTypes.BranchId, branchId.ToString()));
        }
