...
namespace TheCarl.Api.Security;
...
public sealed class HttpCurrentUserContext : ICurrentUserContext
{
    ...
    public Guid OrganizationId => RequireGuidClaim(CarlClaimTypes.OrganizationId);

    public Guid? BranchId
    {
        get
        {
            var raw = Principal?.FindFirst(CarlClaimTypes.BranchId)?.Value;
    ...
    public IReadOnlyCollection<string> Roles
    {
        get
        {
            ...
            return principal.Claims
                .Where(c => c.Type is CarlClaimTypes.Role or ClaimTypes.Role)
                .Select(c => CarlRoles.Normalize(c.Value))
    ...
    public string? SecurityStamp => Principal?.FindFirst(CarlClaimTypes.SecurityStamp)?.Value;
    ...
    public bool HasOrganizationWideScope => Roles.Any(CarlRoles.IsOrganizationWide);
